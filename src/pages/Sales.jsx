...
export default class Sales extends Component {
  ...
  setGraphValues = async () => {
    ...
    // 
    const combined_sales = await axios.get(`${this.state.baseURL}/v1/sales/combined-sales?${this.getRequestQueryParams()}`);
    const {datasets,labels} = combined_sales.data;
    const pkoAccumulated = [];
    const pkcAccumulated = [];
    const pkslAccumulated = [];
    const p2Accumulated = [];

    labels.forEach(element => {
      pkoAccumulated.push(datasets[element].PKO.total_price)
      pkcAccumulated.push(datasets[element].PKC.total_price)
      pkslAccumulated.push(datasets[element].PKSL.total_price)
      p2Accumulated.push(datasets[element].PKC.total_price)
    })
